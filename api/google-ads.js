...
// ─── DIAGNOSTICO DE LA CUENTA (se sirve en el mismo endpoint con ?diag=1) ───────
// Va aqui y no en un fichero aparte porque el plan solo admite 12 funciones y ya
// estaban las 12: un api/google-campanas.js nuevo hacia fallar el despliegue entero.
...
export default async function handler(req, res) {
  ...
  const { from, to, diag } = req.query;
  if (diag) {
    let d;
    try { d = await googleCampanas(from, to); }
    catch (e) { d = { ok: false, error: e.message }; }
    res.setHeader('Cache-Control', d.ok ? 's-maxage=300, stale-while-revalidate=600' : 'no-store');
